...
export const isNotConfigured = (url: string): boolean => {
...
// failure into "Network request failed". Shared by the settings screen's
// BackendStatus card and the home screen's pull-to-refresh.
export function useBackendHealth(backendUrl: string, jwtToken: string | null) {
  const [healthStatus, setHealthStatus] = useState<HealthStatus>({
    status: 'unknown',
    message: 'Not checked',
  });

  const checkBackendHealth = useCallback(async (showAlert: boolean = false) => {
    ...
    console.log('[useBackendHealth] Checking health at:', healthUrl);
    ...
    try {
      ...
      console.log('[useBackendHealth] Health check error:', error);
  ...
  // Auto-check (debounced) whenever the URL or token changes.
  useEffect(() => {
    if (backendUrl.trim()) {
      const timer = setTimeout(() => { checkBackendHealth(false); }, 500);
      return () => clearTimeout(timer);
    }
  }, [backendUrl, jwtToken, checkBackendHealth]);

  return { healthStatus, checkBackendHealth };
}
