"""UI-free BLE device management for the wearable client.

SharedState + AsyncioThread + BLEManager, extracted from the rumps menu bar app
so any front-end (the macOS menu bar app, the unified Chronicle tray) can drive
scanning/connecting/streaming without pulling in a UI toolkit. BLE runs on a
background asyncio thread; the UI polls SharedState snapshots.
"""
...
@dataclass
class SharedState:
    """Thread-safe state shared between the UI and the asyncio BLE thread."""
