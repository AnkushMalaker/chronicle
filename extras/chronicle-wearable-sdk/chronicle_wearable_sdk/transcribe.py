...
READ_TIMEOUT_SECONDS = (
    15.0  # Timeout for waiting for transcript after AudioStop # Edited value
)
                    ...
                    try:
                        async for msg in ws:
                            try:
                                ...
                                if (
                                    "channel" in response
                                    and "alternatives" in response["channel"]
                                ):
                                    transcript = response["channel"]["alternatives"][
                                        0
                                    ].get("transcript", "")
...
async def transcribe_wyoming(audio_queue: Queue, wyoming_url: str):
    ...
    while True:  # Loop for handling audio segments (and potential queue termination)
        ...
        try:
            ...
            logger.debug(
                f"Wyoming: Sending audio chunks for {WINDOW_SECONDS} seconds..."
            )
            while True:
                ...
                try:
                    ...
                    timeout = max(
                        0.1, WINDOW_SECONDS - time_elapsed
                    )  # Use small minimum timeout
                    ...
                    if chunk is None:  # Handle queue termination signal
                        logger.info(
                            "Audio queue finished during chunk sending. Exiting."
                        )
                    ...
                    logger.debug(
                        "Timeout waiting for audio chunk, window likely finished."
                    )
                    ...
                    logger.error(
                        f"Error getting/sending audio chunk: {e}", exc_info=True
                    )
            ...
            if segment_has_audio:
                ...
                logger.debug(
                    f"Wyoming: Reading events for transcript (timeout={READ_TIMEOUT_SECONDS}s)..."
                )
                try:
                    while True:  # Loop to read events until transcript or timeout/error
                        ...
                        if event is None:
                            logger.warning(
                                "Wyoming connection closed by server unexpectedly while waiting for transcript."
                            )
                        ...
                        logger.debug(
                            f"Wyoming: Received event type: {type(event)}"
                        )  # DEBUG
                        if hasattr(event, "data"):
                            logger.debug(
                                f"Wyoming: Received event data: {event.data}"
                            )  # DEBUG
                        ...
                        if (
                            ...
                            logger.debug(
                                "Breaking read loop after receiving transcript."
                            )
                            break
                            ...
                            logger.debug(
                                f"Received non-transcript/non-error event: type={type(event)}"
                            )
                            # Continue reading other events until transcript/error or timeout

                except asyncio.TimeoutError:
                    logger.warning(
                        f"Timeout waiting for transcript after {READ_TIMEOUT_SECONDS}s."
                    )
                    ...
                    logger.info(
                        f"Wyoming connection closed gracefully/expectedly after sending audio: {close_err}"
                    )
                ...
                logger.info(
                    "Skipping AudioStop and transcript read as no audio was sent in this window."
                )
            ...
            logger.error(
                f"Wyoming connection/websocket error during segment: {conn_err}"
            )
            ...
            logger.error(
                f"Unexpected error during segment processing: {e}", exc_info=True
            )
            ...
            if client:  # Check if client was successfully created
                logger.info(
                    "Attempting to disconnect from Wyoming server for this segment..."
                )
