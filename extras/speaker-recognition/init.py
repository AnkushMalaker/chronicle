...
class SpeakerRecognitionSetup:
    ...
    def setup_https(self):
        ...
        if enable_https:
            ...
            if cert_mode == "static":
                ...
                # reach the tailscaled socket). The backend's startup hook renews the
                # shared certs/server.crt that this service also mounts.
                cert_file = Path(__file__).parent / ".." / ".." / "certs" / "server.crt"
                if not cert_file.exists():
                    self.console.print(
                        "[yellow][WARNING][/yellow] No certificate found in certs/; "
                        "run ./wizard.sh to generate it."
                    )
            ...
            if caddyfile_template.exists():
                try:
                    ...
                    # Static mode serves a host-issued cert file; caddy mode lets Caddy
                    # obtain/renew the cert itself (no tls directive).
                    if cert_mode == "static":
                        caddyfile_content = caddyfile_content.replace(
                    ...
                    self._write_caddy_socket_override(
                        Path(__file__).parent, cert_mode, server_ip
                    )
    ...
    def _write_caddy_socket_override(self, service_dir, cert_mode, server_address):
        """Maintain Caddy's Tailscale mount without replacing local overrides."""
        ...
        mounted = update_caddy_socket_override(
            ...
            enabled=(
                cert_mode == "caddy"
                and server_address.endswith(".ts.net")
                and bool(socket)
            ),
        )
        if mounted:
