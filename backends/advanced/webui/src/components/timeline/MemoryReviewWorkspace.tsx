...
/** URL-addressable home for chronological episode and memory decisions. */
export default function MemoryReviewWorkspace() {
  ...
  const firstUnresolved = queue.data?.find(item => item.state !== 'finalized')?.date
  const lastReviewed = queue.data?.[queue.data.length - 1]?.date
  const day = requestedDay || firstUnresolved || lastReviewed || today
