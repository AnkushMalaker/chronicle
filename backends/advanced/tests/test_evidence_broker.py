...
def fixture_day(monkeypatch):
    ...
    episodes = [
        ...
        _episode("day-row", DAY_START + timedelta(hours=2), pipeline="day"),
        _episode(
            "day-confirmed",
...
@pytest.mark.asyncio
async def test_pinned_episodes_span_both_pipelines(fixture_day):
    bundle = await evidence.load_reconciliation_evidence(
        "user-1", DAY_START, DAY_END, timezone_name=TZ
    )

    assert {row["episode_key"] for row in bundle.pinned_episodes} == {
        "day-confirmed",
        "rolling-pinned",
    }
