...
class TestMemoryEntryTimestamps:
    ...
    def test_memory_entry_auto_initializes_timestamps(self):
        ...
        entry = MemoryEntry(id="test-123", content="Test memory content")
        ...
        assert (
            before_creation <= created_timestamp <= after_creation
        ), "created_at should be within test execution time"
        assert (
            before_creation <= updated_timestamp <= after_creation
        ), "updated_at should be within test execution time"

        # Both should be equal since they're created at the same time
        assert (
            entry.created_at == entry.updated_at
        ), "created_at and updated_at should be equal for new entries"
    ...
    def test_memory_entry_with_created_at_only(self):
        ...
        entry = MemoryEntry(
            id="test-123", content="Test memory content", created_at=custom_timestamp
        )

        assert (
            entry.created_at == custom_timestamp
        ), "created_at should match provided value"
        assert (
            entry.updated_at == custom_timestamp
        ), "updated_at should default to created_at"
    ...
    def test_memory_entry_with_both_timestamps(self):
        ...
        assert (
            entry.created_at == created_timestamp
        ), "created_at should match provided value"
        assert (
            entry.updated_at == updated_timestamp
        ), "updated_at should match provided value"
        assert (
            entry.created_at != entry.updated_at
        ), "timestamps should be different when explicitly set"
    ...
    def test_memory_entry_to_dict_includes_timestamps(self):
        ...
        assert (
            "user_id" in entry_dict
        ), "Dict should contain 'user_id' (extracted from metadata)"

        # Verify timestamp values are present and correct
        assert (
            entry_dict["created_at"] == entry.created_at
        ), "Serialized created_at should match entry"
        assert (
            entry_dict["updated_at"] == entry.updated_at
        ), "Serialized updated_at should match entry"

        # Verify frontend compatibility
        assert (
            entry_dict["memory"] == entry.content
        ), "memory field should match content for frontend"
        assert (
            entry_dict["content"] == entry.content
        ), "content field should match content"
    ...
    def test_memory_entry_with_none_timestamps(self):
        ...
        ), "created_at should be auto-initialized from None"
        assert (
            entry.updated_at is not None
        ), "updated_at should be auto-initialized from None"
        assert (
            entry.created_at == entry.updated_at
        ), "Both timestamps should be equal when auto-initialized"
