            ...
            words.append(
                {
                    ...
                    + local_time,  # Cumulative from stream start
...
def create_final_response(cumulative_offset: float = 0.0) -> tuple[dict, float]:
    ...
    response = {
        ...
        "channel": {
            "alternatives": [
                {"transcript": transcript, "confidence": 0.99, "words": words}
            ]
        },
    }
    return response, new_offset
...
async def handle_client(websocket: WebSocketServerProtocol):
    ...
    try:
        ...
        initial, _ = create_deepgram_response(
            transcript="", is_final=False, cumulative_offset=0.0
        )
        ...
        async for message in websocket:
            ...
            if isinstance(message, bytes):
                ...
                if chunk_count % 50 == 0:
                    ...
                    final, new_offset = create_final_response(
                    ...
                    logger.info(
                        f"Sent periodic final result to {client_id} (offset: {current_offset:.2f}s → {new_offset:.2f}s)"
                    )
                # Send interim results every 10 chunks
                    ...
                    logger.debug(
                        f"Sent interim result to {client_id} (offset: {current_offset:.2f}s → {new_offset:.2f}s)"
                    )
            ...
            # Handle control messages
                ...
                try:
                    ...
                    if msg_type == "CloseStream":
                        ...
                        current_offset = connection_state[client_id][
                            "cumulative_offset"
                        ]
                        final, new_offset = create_final_response(
                            cumulative_offset=current_offset
                        ...
                        await websocket.send(json.dumps(final))
                        logger.info(
                            f"Sent final result to {client_id} (offset: {current_offset:.2f}s → {new_offset:.2f}s): {final['channel']['alternatives'][0]['transcript']}"
                        )
        ...
        if client_id in connection_state:
            ...
            logger.info(
                f"Connection closed: {client_id}, processed {chunk_count} chunks, final cumulative offset: {final_offset:.2f}s"
            )
        else:
            logger.info(
                f"Connection closed: {client_id}, processed {chunk_count} chunks"
            )
