...
# uv run --extra deepgram python3 -m backend.workers.old_audio_stream_worker &
# AUDIO_WORKER_1_PID=$!
# uv run --extra deepgram python3 -m backend.workers.old_audio_stream_worker &
# AUDIO_WORKER_2_PID=$!

# echo "📝 Starting transcription stream workers (2 workers)..."
# uv run --extra deepgram python3 -m backend.workers.old_transcription_stream_worker &
