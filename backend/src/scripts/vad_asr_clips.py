...
from backend.services.audio_claims import locate_conversation_audio_at
from backend.services.timeline.episode_bounds import speech_profile_for_range
from backend.services.transcript_time import as_utc, transcript_for_range
...
async def _locate(when: datetime) -> Optional[tuple[str, float]]:
    ...
    if location is None:
        return None
    return location.conversation_id, location.offset_seconds
