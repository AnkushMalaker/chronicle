...
def get_model_config(config_data: Dict[str, Any], model_role: str) -> Dict[str, Any]:
    ...
    Example:
        ...
        >>> from backend.utils.model_utils import get_model_config
    ...
    if not default_name:
        raise ValueError(
            f"Configuration for 'defaults.{model_role}' not found in config.yml"
        )
    ...
    raise ValueError(
        f"Model '{default_name}' for role '{model_role}' not found in config.yml models list"
    )
