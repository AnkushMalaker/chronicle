...
def _named_entities(episodes):
    return [
        entity
        for episode in episodes
        for entity in episode.entities
        if not re.fullmatch(r"(?:Unknown(?: Speaker)?|Speaker)(?: \d+)?", entity, re.I)
    ]


def _group_title(episodes: list[Any], lane: str) -> str:
    # A single semantic episode already has a title; do not guess a participant
    # from its untyped entity bag (which once produced "with Amazon").
    if len(episodes) == 1 and episodes[0].title.strip():
        return episodes[0].title.strip()
    entities = _named_entities(episodes)
...
def _review_group(
    episodes: list[Any],
    index: int,
    semantic_group: TimelineSemanticGroupRevision | None = None,
) -> dict[str, Any]:
...
def build_day_review_projection(
    ...
    *,
    semantic_group_revisions: Iterable[TimelineSemanticGroupRevision] = (),
    local_date,
    timezone_name: str,
) -> dict[str, Any]:
    ...
    for group in semantic_group_revisions:
        members = [
    ...
    for lane in ("conversation", "foreground", "background"):
        ...
        for episode in (
            ...
            # Adjacency does not mean two conversations are the same session.
            # Only an explicitly accepted semantic group may combine them.
            if lane == "conversation":
