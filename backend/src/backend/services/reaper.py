...
3. **Orphaned deferred jobs** — RQ chain jobs stuck ``deferred`` forever because a
   ...
   *fails*, not one that *vanishes*). See :mod:`backend.services.job_reaper`.
...
from backend.config import WS_IDLE_TIMEOUT_SECS
from backend.redis_factory import create_async_redis
from backend.services.job_reaper import reap_orphaned_deferred_jobs
