...
from backend.speaker_recognition_client import _select_label_mappings
...
from backend.workers.speaker_jobs import (
...
def test_archived_speaker_history_is_removed_from_embedded_projection(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.workers.speaker_jobs." "ConversationTranscriptRevision.find_one",
        archived,
    )
