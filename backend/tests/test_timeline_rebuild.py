...
async def test_registered_day_worker_prepares_review_without_writing_the_vault(
    ...
    class Request:
        ...
        async def insert(self):
            calls.append(("insert", self.request_id))
            return self
    ...
    async def authorize(**kwargs):
        calls.append(("authorize", kwargs["source_kind"], kwargs["reason"]))
        return SimpleNamespace(dirty_range_id="dirty-1")

    async def reconcile(request):
        calls.append(("reconcile", request.dirty_range_id))
        return {"request_id": request.request_id, "state": "completed"}

    monkeypatch.setattr(timeline_jobs, "TimelineReconciliationRequest", Request)
    monkeypatch.setattr(timeline_jobs, "authorize_explicit_range", authorize)
    monkeypatch.setattr(timeline_jobs, "_run_explicit_reconciliation", reconcile)

    result = await timeline_jobs.rebuild_timeline_day_job.__wrapped__(
        USER, "2026-08-06", "Asia/Kolkata", "rebuild-1", redis_client=None
    )

    assert calls[1:] == [
        ("authorize", "rebuild", "timeline_rebuild"),
        ("reconcile", "dirty-1"),
    ]
