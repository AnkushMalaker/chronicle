...
from backend.routers.modules import health_routes
from backend.services.observability import health_poller
from backend.workers.orchestrator.health_monitor import HealthMonitor
...
def test_health_monitor_publishes_actual_child_process_counts(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.workers.orchestrator.health_monitor.time.time",
        lambda: 1_000,
    )
