...
from backend.integrations.swiggy import client as client_module
from backend.integrations.swiggy.client import ToolResult
...
async def test_safe_read_retries_transient_failures(monkeypatch):
    ...
    sleep = AsyncMock()
    monkeypatch.setattr("backend.integrations.swiggy.client.asyncio.sleep", sleep)
