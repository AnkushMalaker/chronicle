...
start_workers() {
    ...
    if python3 -m backend.workers.audio_stream_worker &
    then
        AUDIO_WORKER_1_PID=$!
    ...
    if python3 -m backend.workers.rq_worker_entry transcription memory default &
    then
        RQ_WORKER_1_PID=$!
    ...
    if python3 -m backend.workers.rq_worker_entry summary &
    then
        SUMMARY_WORKER_PID=$!
    ...
    if python3 -m backend.workers.rq_worker_entry audio &
    then
...
# if uv run --no-sync python3 -m backend.workers.old_audio_stream_worker &
# then
#     AUDIO_WORKER_1_PID=$!
...
# if uv run --no-sync python3 -m backend.workers.old_transcription_stream_worker &
# then
...
if python3 src/backend/main.py &
then
    BACKEND_PID=$!
